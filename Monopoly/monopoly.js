function Game() {
	...
	// Function to get the value of a die
	...
	var finalizeAuction = function() {
		...
		if (highestbid > 0) {
			...
			addAlert(p.name + " bought " + sq.name + " for Rs" + highestbid + ".");
		}
	...
	// Function to add a property to the auction queue
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Auction <span id='propertyname'></span></div><div>Highest Bid = Rs<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, it is your turn to bid.</div<div><input id='bid' title='Enter an amount to bid on " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Bid' onclick='game.auctionBid();' title='Place your bid.' /><input type='button' value='Pass' title='Skip bidding this time.' onclick='game.auctionPass();' /><input type='button' value='Exit Auction' title='Stop bidding on " + s.name + " altogether.' onclick='if (confirm(\"Are you sure you want to stop bidding on this property altogether?\")) game.auctionExit();' /></div>", "blank");
	...
	// Function for a player to pass in an auction
			...
			if (currentbidder == highestbidder) {
				...
				if (!p.human) {
					...
					if (bid === -1 || highestbid >= p.money) {
						...
						this.auctionBid(bid);
						window.alert(p.name + " bid Rs" + bid + ".");
						continue;
					}
					return;
				} else {
	...
	this.auctionBid = function(bid) {
		...
		if (bid === "" || bid === null) {
			...
			if (bid > player[currentbidder].money) {
				document.getElementById("bid").value = "You don't have enough money to bid Rs" + bid + ".";
				document.getElementById("bid").style.color = "red";
				...
				document.getElementById("bid").value = "Your bid must be greater than highest bid. (Rs" + highestbid + ")";
				document.getElementById("bid").style.color = "red";
			}
		}
	};

	// Function to exit the auction for a player
	this.auctionExit = function() {
		...
		// Set event handlers for trade money inputs
		...
		// Function to reset the trade interface with new initiator and recipient
		var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
			...
			var tableRowOnClick = function(e) {
				// Toggle checkbox if the clicked element is not the checkbox itself
			...
			// Reset the trade initiator and recipient
			currentInitiator = initiator;
			currentRecipient = recipient;
	
			// Clear property elements
			...
			// Create tables for initiator and recipient properties
			...
			for (var i = 0; i < 40; i++) {
				...
				// Skip properties that cannot be traded
				...
				// Add properties to initiator and recipient side tables
				if (currentSquare.owner === initiator.index) {
					// Add row for initiator property
				...
				// Requested properties.
					...
					// Add row for recipient property
			...
			// Add Get Out of Jail Free Cards to tables if available
	...
	this.bankruptcyUnmortgage = function() {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == p.index && sq.mortgage) {
				...
				HTML += "' onmouseover='showdeed(" + i + ");' onmouseout='hidedeed();'></td><td class='propertycellname'><a href='javascript:void(0);' title='Unmortgage " + sq.name + " for Rs" + price + ".' onclick='if (" + price + " <= player[" + p.creditor + "].money) {player[" + p.creditor + "].pay(" + price + ", 0); square[" + i + "].mortgage = false; addAlert(\"" + player[p.creditor].name + " unmortgaged " + sq.name + " for Rs" + price + ".\");} this.parentElement.parentElement.style.display = \"none\";'>Unmortgage " + sq.name + " (Rs" + price + ")</a></td></tr>";
	...
	this.bankruptcy = function() {
		...
		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			...
			addAlert(pcredit.name + " paid Rs" + bankruptcyUnmortgageFee + " interest on the mortgaged properties received from " + p.name + ".");
			popup("<p>" + pcredit.name + ", you must pay Rs" + bankruptcyUnmortgageFee + " interest on the mortgaged properties you received from " + p.name + ".</p>", function() {player[pcredit.index].pay(bankruptcyUnmortgageFee, 0); game.bankruptcyUnmortgage();});
		}
	};
...
function updateMoney() {
	...
	document.getElementById("pmoney").innerHTML = "Rs" + p.money;
	$(".money-bar-row").hide();
...
function updateOption() {
	...
	if (sq.mortgage) {
		document.getElementById("mortgagebutton").value = "Unmortgage (Rs" + Math.round(sq.price * 0.55) + ")";
		document.getElementById("mortgagebutton").title = "Unmortgage " + sq.name + " for Rs" + Math.round(sq.price * 0.55) + ".";
		$("#buyhousebutton").hide();
		$("#sellhousebutton").hide();

		allGroupUnmortgaged = false;
	} else {
		document.getElementById("mortgagebutton").value = "Mortgage (Rs" + (sq.price * 0.5) + ")";
		document.getElementById("mortgagebutton").title = "Mortgage " + sq.name + " for Rs" + (sq.price * 0.5) + ".";
		...
		if (sq.groupNumber >= 3) {
			...
			buyhousebutton.value = "Buy house (Rs" + sq.houseprice + ")";
			sellhousebutton.value = "Sell house (Rs" + (sq.houseprice * 0.5) + ")";
			buyhousebutton.title = "Buy a house for Rs" + sq.houseprice;
			sellhousebutton.title = "Sell a house for Rs" + (sq.houseprice * 0.5);

			if (sq.house == 4) {
				buyhousebutton.value = "Buy hotel (Rs" + sq.houseprice + ")";
				buyhousebutton.title = "Buy a hotel for Rs" + sq.houseprice;
			}
			if (sq.hotel == 1) {
				$("#buyhousebutton").hide();
				sellhousebutton.value = "Sell hotel (Rs" + (sq.houseprice * 0.5) + ")";
				sellhousebutton.title = "Sell a hotel for Rs" + (sq.houseprice * 0.5);
			}
...
function addamount(amount, cause) {
	...
	addAlert(p.name + " received Rs" + amount + " from " + cause + ".");
}
...
function subtractamount(amount, cause) {
	...
	addAlert(p.name + " lost Rs" + amount + " from " + cause + ".");
}
...
function payeachplayer(amount, cause) {
	...
	addAlert(p.name + " lost Rs" + total + " from " + cause + ".");
}
...
function collectfromeachplayer(amount, cause) {
	...
	addAlert(p.name + " received Rs" + total + " from " + cause + ".");
}
...
function streetrepairs(houseprice, hotelprice) {
	...
	if (cost > 0) {
		...
		if (houseprice === 40) {
			addAlert(p.name + " lost Rs" + cost + " to Community Chest.");
		} else {
			addAlert(p.name + " lost Rs" + cost + " to Chance.");
		}
	}
	...
	for (var x = 1; x <= pcount; x++) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == x) {
				...
				HTML += "' onmouseover='showdeed(" + i + ");' onmouseout='hidedeed();'></td><td class='statscellname' " + mortgagetext + ">" + sq.name + "(Rs" + sq.price + ")" + housetext + "</td></tr>";
			}
		}
...
function buy() {
	...
	if (p.money >= cost) {
		...
		popup("<p>" + p.name + ", you need Rs" + (property.price - p.money) + " more to buy " + property.name + ".</p>");
	}
}
...
function mortgage(index) {
	...
	document.getElementById("mortgagebutton").value = "Unmortgage for Rs" + unmortgagePrice;
	document.getElementById("mortgagebutton").title = "Unmortgage " + sq.name + " for Rs" + unmortgagePrice + ".";

	addAlert(p.name + " mortgaged " + sq.name + " for Rs" + mortgagePrice + ".");
	updateOwned();
	updateMoney();
...
function unmortgage(index) {
	...
	sq.mortgage = false;
	document.getElementById("mortgagebutton").value = "Mortgage for Rs" + mortgagePrice;
	document.getElementById("mortgagebutton").title = "Mortgage " + sq.name + " for Rs" + mortgagePrice + ".";

	addAlert(p.name + " unmortgaged " + sq.name + " for Rs" + unmortgagePrice + ".");
	updateOwned();
	return true;
}
...
function land(increasedRent) {
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>You landed on <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Buy (Rs" + s.price + ")' title='Buy " + s.name + " for " + s.pricetext + ".'/></div>";
		}
	...
	if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
		...
		addAlert(p.name + " paid Rs" + rent + " rent to " + player[s.owner].name + ".");
		p.pay(rent, s.owner);
		player[s.owner].money += rent;

		document.getElementById("landed").innerHTML = "You landed on " + s.name + ". " + player[s.owner].name + " collected Rs" + rent + " rent.";
...
function play() {
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "You are in jail.<input type='button' title='Pay Rs50 fine to get out of jail immediately.' value='Pay $50 fine' onclick='payfifty();' />";
...
window.onload = function() {
	...
	$("#mortgagebutton").click(function() {
		...
		if (s.mortgage) {
			if (player[s.owner].money < Math.round(s.price * 0.55)) {
				popup("<p>You need Rs" + (Math.round(s.price * 0.55) - player[s.owner].money) + " more to unmortgage " + s.name + ".</p>");

			} else {
				popup("<p>" + player[s.owner].name + ", are you sure you want to unmortgage " + s.name + " for Rs" + Math.round(s.price * 0.55) + "?</p>", function() {
					unmortgage(checkedProperty);
				}, "Yes/No");
			}
		} else {
			popup("<p>" + player[s.owner].name + ", are you sure you want to mortgage " + s.name + " for Rs" + Math.round(s.price * 0.5) + "?</p>", function() {
	...
	$("#buyhousebutton").on("click", function() {
		...
		if (p.money < s.houseprice) {
			if (s.house === 4) {
				popup("<p>You need Rs" + (s.houseprice - player[s.owner].money) + " more to buy a hotel for " + s.name + ".</p>");
				return;
			} else {
				popup("<p>You need Rs" + (s.houseprice - player[s.owner].money) + " more to buy a house for " + s.name + ".</p>");
				return;
			}
		}
